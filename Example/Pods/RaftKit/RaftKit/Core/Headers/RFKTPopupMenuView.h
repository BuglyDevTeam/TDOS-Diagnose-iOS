 ...
 *
 *  注意，RFKTPopupMenuView 的大小默认是按内容自适应的（item 的 sizeThatFits），但同时又受 maximumWidth/minimumWidth 的限制。
 */
@interface RFKTPopupMenuView : RFKTPopupContainerView
...
/// 批量设置 item 的样式
@property(nonatomic, copy, nullable) void (^itemConfigurationHandler)(RFKTPopupMenuView *aMenuView, __kindof RFKTPopupMenuBaseItem *aItem, NSInteger section, NSInteger index);

/// 设置 item，均处于同一个 section 内
@property(nonatomic, copy, nullable) NSArray<__kindof RFKTPopupMenuBaseItem *> *items;

/// 设置多个 section 的多个 item
@property(nonatomic, copy, nullable) NSArray<NSArray<__kindof RFKTPopupMenuBaseItem *> *> *itemSections;
