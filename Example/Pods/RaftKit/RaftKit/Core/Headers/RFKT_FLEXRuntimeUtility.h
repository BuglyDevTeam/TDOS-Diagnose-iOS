...
#define PropertyKey(suffix) kRFKT_FLEXPropertyAttributeKey##suffix : @""
#define PropertyKeyGetter(getter) kRFKT_FLEXPropertyAttributeKeyCustomGetter : NSStringFromSelector(@selector(getter))
#define PropertyKeySetter(setter) kRFKT_FLEXPropertyAttributeKeyCustomSetter : NSStringFromSelector(@selector(setter))

/// Takes: min iOS version, property name, target class, property type, and a list of attributes
#define RFKT_FLEXRuntimeUtilityTryAddProperty(iOS_atLeast, name, cls, type, ...) ({ \
    if (@available(iOS iOS_atLeast, *)) { \
        NSMutableDictionary *attrs = [NSMutableDictionary dictionaryWithDictionary:@{ \
            kRFKT_FLEXPropertyAttributeKeyTypeEncoding : @(type), \
            __VA_ARGS__ \
        }]; \
        [RFKT_FLEXRuntimeUtility \
...
/// Takes: min iOS version, property name, target class, property type, and a list of attributes
#define RFKT_FLEXRuntimeUtilityTryAddNonatomicProperty(iOS_atLeast, name, cls, type, ...) \
    RFKT_FLEXRuntimeUtilityTryAddProperty(iOS_atLeast, name, cls, @encode(type), PropertyKey(NonAtomic), __VA_ARGS__);
/// Takes: min iOS version, property name, target class, property type (class name), and a list of attributes
#define RFKT_FLEXRuntimeUtilityTryAddObjectProperty(iOS_atLeast, name, cls, type, ...) \
    RFKT_FLEXRuntimeUtilityTryAddProperty(iOS_atLeast, name, cls, RFKT_FLEXEncodeClass(type), PropertyKey(NonAtomic), __VA_ARGS__);

@interface RFKT_FLEXRuntimeUtility : NSObject
...
+ (id)potentiallyUnwrapBoxedPointer:(id)returnedObjectOrNil type:(const RFKT_FLEXTypeEncoding *)returnType;
...
+ (NSUInteger)fieldNameOffsetForTypeEncoding:(const RFKT_FLEXTypeEncoding *)typeEncoding;
