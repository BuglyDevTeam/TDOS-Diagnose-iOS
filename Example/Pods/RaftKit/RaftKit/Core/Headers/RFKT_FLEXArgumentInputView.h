...
typedef NS_ENUM(NSUInteger, RFKT_FLEXArgumentInputViewSize) {
    /// 2 lines, medium-sized
    RFKT_FLEXArgumentInputViewSizeDefault = 0,
    /// One line
    RFKT_FLEXArgumentInputViewSizeSmall,
    /// Several lines
    RFKT_FLEXArgumentInputViewSizeLarge
};

@protocol RFKT_FLEXArgumentInputViewDelegate;

@interface RFKT_FLEXArgumentInputView : UIView
...
@property (nonatomic) RFKT_FLEXArgumentInputViewSize targetSize;

/// Users of the input view can get delegate callbacks for incremental changes in user input.
@property (nonatomic, weak) id <RFKT_FLEXArgumentInputViewDelegate> delegate;
...
/// Used by RFKT_FLEXArgumentInputViewFactory to create appropriate input views.
...
@protocol RFKT_FLEXArgumentInputViewDelegate <NSObject>

- (void)argumentInputViewValueDidChange:(RFKT_FLEXArgumentInputView *)argumentInputView;
