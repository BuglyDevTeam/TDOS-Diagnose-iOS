...
/**
 *  配合 RFKTPopupMenuView 使用，用于可点击的菜单项。
 *  支持显示图片和标题，以及点击事件的回调。
 *  可在 RFKTPopupMenuView 里统一修改菜单项的样式，如果某个菜单项需要特殊调整，可通过 RFKTPopupMenuButtonItem.button 拿到 view 并进行调整。
 */
@interface RFKTPopupMenuButtonItem : RFKTPopupMenuBaseItem

/// item 里的图片，默认在左边，也可通过 item.button.imagePosition 修改图片的位置
@property(nonatomic, strong, nullable) UIImage *image;

/// 增加title color属性，支持menu button设置不同颜色
@property (nonatomic, strong, nullable) UIColor *titleColor;

/// 每个 item 都通过一个 button 来显示内容，可直接修改 button 的相关属性达到自定义的效果。
@property(nonatomic, strong, readonly, nonnull) UIButton *button;
...
+ (instancetype)itemWithImage:(nullable UIImage *)image
                      ...
                      handler:(nullable void (^)(RFKTPopupMenuButtonItem *aItem))handler;
