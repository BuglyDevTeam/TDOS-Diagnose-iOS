...
typedef void (^RFKT_FLEXMutableListCellForElement)(__kindof UITableViewCell *cell, id element, NSInteger row);
...
///
/// By default, \c kRFKT_FLEXDetailCell is the reuse identifier used. If you need
...
@interface RFKT_FLEXMutableListSection<__covariant ObjectType> : RFKT_FLEXCollectionContentSection

/// Initializes a section with an empty list.
+ (instancetype)list:(NSArray<ObjectType> *)list
   cellConfiguration:(RFKT_FLEXMutableListCellForElement)configurationBlock
...
/// A readwrite version of the same property in \c RFKT_FLEXTableViewSection.h
///
