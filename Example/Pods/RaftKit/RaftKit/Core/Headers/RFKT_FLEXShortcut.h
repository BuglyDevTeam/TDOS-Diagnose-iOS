...
/// subset of the responsibilities of a \c RFKT_FLEXShortcutsSection
...
@protocol RFKT_FLEXShortcut <RFKT_FLEXObjectExplorerItem>
...
- (UIViewController *)editorWith:(id)object forSection:(RFKT_FLEXTableViewSection *)section;

@end


/// Provides default behavior for RFKT_FLEX metadata objects. Also works in a limited way with strings.
/// Used internally. If you wish to use this object, only pass in \c RFKT_FLEX* metadata objects.
@interface RFKT_FLEXShortcut : NSObject <RFKT_FLEXShortcut>

/// @param item An \c NSString or \c RFKT_FLEX* metadata object.
/// @note You may also pass a \c RFKT_FLEXShortcut conforming object,
/// and that object will be returned instead.
+ (id<RFKT_FLEXShortcut>)shortcutFor:(id)item;

@end


/// Provides a quick and dirty implementation of the \c RFKT_FLEXShortcut protocol,
/// allowing you to specify a static title and dynamic atttributes for everything else.
/// The object passed into each block is the object passed to each \c RFKT_FLEXShortcut method.
///
/// Does not support the \c -editorWith: method.
@interface RFKT_FLEXActionShortcut : NSObject <RFKT_FLEXShortcut>
